...
public class MyHashMap<K extends Comparable<K>, V> implements MyMap<K, V> {
     ...
     *         mapping for key
     */
    //call key.hashcode() to get key's hashcode
    //mod hashcode by table size to determine key where the entry should be inserted
    //managing next pointers
    //if key is found in table, update value and return old value associated the key
    //else {if key is not found, insert key-value pair and return null
    //If another entry hashes to the same index in the table, insert it at the head of the list at that index
    //2.next = 1
...
//        System.out.println("h");
    ...
    private void rehash() {
        ...
        Entry<K, V>[] newHash = new Entry[primes[++primeIndex]];
        for(int i = 0; i<table.length; i++){
            if(table[i] != null) {
                Entry<K, V> ta = table[i];
                while (ta != null) {
//                    System.out.println("e: " + e);
                    int index1 = ta.key.hashCode() % newHash.length;
                    Entry<K, V> t = new Entry<>(ta.key, ta.value);
                    t.next = newHash[index1];
                    newHash[index1] = t;
                    ta = ta.next;
                }
            }
...
//        Entry<K,V> e = new Entry<>(key, value);
    ...
    public static void main(String[] args) {
        ...
        int upperLimit = 100;
        int expectedSum = 0;
        System.out.println("Hash Map before put");
        ...
        System.out.println("Hash Map after put");
        System.out.println("Size            : " + map.size());
        System.out.println("Table size      : " + map.getTableSize());
        System.out.println("Load factor     : " + map.getLoadFactor());
        System.out.println("Max chain length: " + map.computeMaxChainLength());
